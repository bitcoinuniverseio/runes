...
import { decipher, encipher, parseRawTransaction, simulateAllocation, analyzeSize, parseHex, } from '@bitcoinuniverse/runes-tools';
    ...
    switch (name) {
        ...
        case 'encode_runestone': {
            ...
            if (args.pointer !== undefined)
                input.pointer = Number(args.pointer);
            if (args.edicts) {
                input.edicts = args.edicts.map((e) => ({
        ...
        case 'simulate_allocation': {
            ...
            for (const [k, v] of Object.entries(args.inputBalances || {})) {
                inputBalances[k] = BigInt(v);
            }
            const edicts = (args.edicts || []).map((e) => ({
                id: { block: BigInt(e.id.block), tx: Number(e.id.tx) },
                amount: BigInt(e.amount),
                output: Number(e.output)
            }));
        ...
        case 'explain_bytes': {
            ...
            for (let i = 0; i < clean.length; i += 2) {
                ...
                let note = `Offset ${i / 2}`;
                if (i === 0 && hex.toLowerCase() === '6a')
                    note = 'OP_RETURN (0x6a)';
                else if (i === 2 && hex.toLowerCase() === '5d')
                    note = 'OP_13 (0x5d) Protocol ID';
        ...
        case 'query_protocol': {
            ...
            const matchedRules = rulesData.filter((r) => r.title.toLowerCase().includes(q) || r.text.toLowerCase().includes(q)).slice(0, 5);
            const matchedFlaws = flawsData.filter((f) => f.flaw.toLowerCase().includes(q) || f.condition.toLowerCase().includes(q));
            return { query: q, matchedRules, matchedFlaws };
        }
        case 'get_provenance': {
            if (args.ruleId) {
                const q = String(args.ruleId).toUpperCase().trim();
                const r = provenanceData.records.find((x) => x.ruleId === q);
