...
export function nextRuneId(id, blockDelta, txDelta) {
    const block = id.block + blockDelta;
    if (block > U64_MAX || block < 0n)
        return null;
    let tx;
    if (blockDelta === 0n) {
...
export function deltaRuneId(previous, target) {
...
export function formatRuneId(id) {
    return `${id.block}:${id.tx}`;
}
export function parseRuneId(s) {
    ...
    try {
        ...
        if (isNaN(tx) || tx < 0 || tx > Number(U32_MAX))
            return null;
        if (block < 0n || block > U64_MAX)
            return null;
        if (block === 0n && tx > 0)
            return null;
        return { block, tx };
