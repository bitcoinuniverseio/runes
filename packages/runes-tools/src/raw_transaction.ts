...
import { decipher, DecipherResult } from './runestone.js';
...
function readVarInt(bytes: Uint8Array, offset: number): { value: bigint; length: number } {
  if (offset >= bytes.length) throw new Error('Unexpected EOF reading varint');
  const first = bytes[offset];
  if (first < 0xfd) {
    return { value: BigInt(first), length: 1 };
  } else if (first === 0xfd) {
    if (offset + 3 > bytes.length) throw new Error('Unexpected EOF reading varint 0xfd');
    const val = bytes[offset + 1] | (bytes[offset + 2] << 8);
    return { value: BigInt(val), length: 3 };
  } else if (first === 0xfe) {
    if (offset + 5 > bytes.length) throw new Error('Unexpected EOF reading varint 0xfe');
    let val = 0n;
    for (let i = 0; i < 4; i++) {
    ...
    if (offset + 9 > bytes.length) throw new Error('Unexpected EOF reading varint 0xff');
    let val = 0n;
    for (let i = 0; i < 8; i++) {
      val |= BigInt(bytes[offset + 1 + i]) << BigInt(8 * i);
    }
...
export function parseRawTransaction(rawHex: string): RawTransactionResult {
  ...
  for (let i = 0; i < vinCount; i++) {
    if (offset + 36 > bytes.length) throw new Error(`Unexpected EOF reading input ${i}`);
    ...
    if (offset + scriptLen > bytes.length) throw new Error(`Unexpected EOF reading scriptSig in input ${i}`);
  ...
  const nonOpReturnOutputIndices: number[] = [];
  let runestoneOutputIndex: number | null = null;

  for (let i = 0; i < voutCount; i++) {
    if (offset + 8 > bytes.length) throw new Error(`Unexpected EOF reading value for output ${i}`);
    let valueSats = 0n;
    for (let b = 0; b < 8; b++) {
    ...
    if (offset + scriptLen > bytes.length) throw new Error(`Unexpected EOF reading scriptPubKey for output ${i}`);
  ...
  if (isSegwit) {
    for (let i = 0; i < vinCount; i++) {
      ...
      for (let j = 0; j < itemCount; j++) {
        ...
        if (offset + itemLen > bytes.length) throw new Error(`Unexpected EOF reading witness item in input ${i}`);
  ...
  // Locktime: 4 bytes LE
  if (offset + 4 > bytes.length) throw new Error('Unexpected EOF reading locktime');
  const locktime = bytes[offset] | (bytes[offset + 1] << 8) | (bytes[offset + 2] << 16) | (bytes[offset + 3] << 24);

  // Decipher runestone if found
  let runestone: DecipherResult | null = null;
