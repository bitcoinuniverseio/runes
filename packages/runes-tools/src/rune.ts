...
export function encodeRuneName(s: string): bigint {
...
export function formatSpacedName(nameValue: bigint, spacers: bigint): string {
...
export function parseSpacedName(text: string): { name: string; nameValue: bigint; spacers: bigint } {
  let name = '';
  let spacers = 0n;
 ...
 */
export function getRuneCommitmentBytes(nameValue: bigint): Uint8Array {
  if (nameValue === 0n) {
...
export function minimumLengthAtHeight(height: number): number {
...
export function isNameUnlocked(name: string, height: number): boolean {
  const clean = name.replace(/[^A-Za-z]/g, '');
  const minLen = minimumLengthAtHeight(height);
  return clean.length >= minLen;
}

export function isUnicodeScalar(v: bigint): boolean {
  if (v > 0x10ffffn || v < 0n) return false;
