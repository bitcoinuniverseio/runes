...
import { OP_RETURN, MAGIC_NUMBER, MAX_SCRIPT_ELEMENT_SIZE, FlawType } from './constants.js';

export function parseHex(text: string): { bytes: Uint8Array } | { error: string } {
...
export function extractPayload(bytes: Uint8Array): ExtractPayloadResult {
  if (bytes.length < 1 || bytes[0] !== OP_RETURN) {
    return {
      notRunestone:
        "Script does not begin with OP_RETURN (0x6a). This output carries no runestone; indexers skip it and examine the transaction's other outputs.",
    };
  }
  if (bytes.length < 2 || bytes[1] !== MAGIC_NUMBER) {
    return {
      notRunestone:
        'OP_RETURN is not followed by OP_13 (0x5d), the Runes protocol identifier. This is an ordinary data carrier output, not a runestone.',
    };
  }
...
export function buildRunestoneScript(payload: Uint8Array): Uint8Array {
  const parts: number[] = [OP_RETURN, MAGIC_NUMBER];
